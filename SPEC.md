...
**Timeline focus (v1.2, second `<script>` block):** the row-label column is sticky, the board opens scrolled so the today line is centered, and while the timeline scrolls horizontally, any work row whose bar lies entirely outside the visible weeks is collapsed (a lane collapses too once nothing in it is visible). A "Timeline focus: on/off" button in the topbar disables the filtering. The script is board-agnostic — it reads the column count and label width from the DOM, and on a viewport wide enough to show every column it never hides anything. Nothing to configure per client; carry the block forward verbatim.

**Engagement-length variant:** the standard board is one quarter (16 columns), but a board may instead span the whole engagement — one column per week from kickoff to SOW end (reference: the Willie's Remedy board, 29 columns). Same encoding throughout; change `repeat(16, 1fr)`, every cell run, sprint bands, and week labels together, raise the grid's `min-width` so columns stay readable (~90px per column), and keep `prior` shading on all elapsed weeks. Timeline focus is what makes this variant usable — the scroll window plays the role the quarter roll plays on the standard board. Prefer the quarterly board for ongoing retainers (bounded history, cheap updates); use engagement-length when the client should see the whole arc on one page. Bars that straddle the line get `crosses` + an inline `--past: N%` (percentage of the bar already elapsed — recompute it whenever you move/extend a crossing bar).
