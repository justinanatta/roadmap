...
3. **For each new item** surfaced: if it belongs to an existing (public) lane, add a row + bar there with the correct pill, `proposed` (dashed) until it has a real sprint, then solid once scheduled. Copy the 16-cell filler run exactly from a neighboring row. If instead the source floats a **whole net-new workstream not yet confirmed with the client**, stage it as a **draft lane** — `<details class="group draft" open>` (SPEC §4) — visible on the review branch but never merged to main. Draft lanes are the *only* place for unconfirmed/internal work; one item added to a public lane is a normal edit, not a draft. The guardrails above apply inside draft lanes too. Resolve draft lanes at publish time with `/roadmap-publish`.
